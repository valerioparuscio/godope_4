...
def test_has_free_rat_slot_and_confiscation_slot_track_independently(game_data) -> None:
    ...
    events: list = []
    for _ in range(len(state.jail.slots)):
        jail.confiscate_dope(state, DopeType.RANA, events)
    assert not jail.has_free_confiscation_slot(state)
    assert jail.has_free_rat_slot(state)


def test_last_rat_triggers_evasion_and_returns_others_to_base(game_data) -> None:
    state, _ = _new_game(game_data)
    player = state.players[0]
    ...
    assert all(state.pawns[pid].role == PawnRole.RAT for pid in pawn_ids[:-1])
...
def test_own_rats_job_completes_even_when_the_last_rat_triggers_evasion(game_data) -> None:
    ...
    correctly but never completed Job 4 ("Abbi 2 Rats", was "Abbi 3
    Rats") — the post-success hook that normally checks completion only
    runs once, at the very end of the whole command, by which point
    Evasion had already returned every Rat (this player's included) back
    to base, so the live snapshot no longer showed enough of them.
    rules/jail.py now checks completion itself at the one moment the
    snapshot can be true: right as the last slot fills, before Evasion
    undoes it."""
    state, _ = _new_game(game_data)
    job = next(j for j in game_data.jobs if j.requirement["type"] == "own_rats")
    required = job.requirement["count"]
    slot_count = len(state.jail.slots)
    other_arrests_needed = slot_count - required
    assert 0 < other_arrests_needed <= 3, "test assumes 3 other players can each supply 1 Rat"
    player_0 = state.players[0]
    _set_revealed_job(state, game_data, player_0.player_id, job.job_id)
    events: list = []

    # Fill every slot but the last one, giving player 0 one fewer Rat
    # than the Job needs — the rest come from the other 3 players' own
    # pawns, so player 0 isn't at the requirement yet.
    for other_player in state.players[1 : 1 + other_arrests_needed]:
        jail.arrest_pawn(state, other_player.pawn_ids[0], events)
    for pawn_id in player_0.pawn_ids[: required - 1]:
        jail.arrest_pawn(state, pawn_id, events)
    assert not any(type(e).__name__ == "JobCompleted" for e in events)

    # The last arrest overall is player 0's own last-needed Rat, and also
    # the one that fills the last slot and triggers Evasion.
    jail.arrest_pawn(state, player_0.pawn_ids[required - 1], events)
