...
def test_view_hides_other_players_hand_contents(game_data) -> None:
    ...
    view = build_player_view(state, viewer_id)
...
def test_view_only_exposes_pending_decision_to_its_own_player(game_data) -> None:
    state, _ = create_initial_state(game_data, game_id=GameId("g"), seed=1, human_seat=0)
    ...
    own_view = build_player_view(state, state.current_player_id)
    other_view = build_player_view(state, other_id)
